...
# 重置 genesis，确保 chainId 为 TARGET_CHAIN_ID
function reset_genesis() {
    ...
    # 强制设置 genesis-template.json 中的 chainId
    sed -i -e "s/\"chainId\": [0-9]*/\"chainId\": ${TARGET_CHAIN_ID}/" ${workspace}/genesis/genesis-template.json
...
function prepare_config() {
    ...
    # 确保生成的 genesis.json 中的 chainId 为 TARGET_CHAIN_ID
    sed -i -e "s/\"chainId\": [0-9]*/\"chainId\": ${TARGET_CHAIN_ID}/" ${workspace}/genesis/genesis.json
}
